...
public interface IGuestStore
{
    ...
    Task<IReadOnlyList<(string Link, string FirstName)>> ListAsync(CancellationToken ct = default);
    Task CreateAsync(string link, string firstName, CancellationToken ct = default);
    Task UpdateProfileAsync(string link, string? nickname, AttendingStatus? attending, CancellationToken ct = default);
